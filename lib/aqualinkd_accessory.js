...
function AqualinkdAccessory(platform, platformAccessory, id, device, uuid) {
  ...
  this.value = this.setTemperatureValue(Utils.parseAccessoryValue(this.type, device.value));
...
AqualinkdAccessory.prototype = {
  ...
  getServices : function() {
    ...
    if (this.type === Constants.adDeviceSwitch || this.type === Constants.adDevicePrgSwitch) {
      ...
      if (this.type === Constants.adDeviceSWGp) {
         ...
         validValues = [0,2]; // Don't allow SWG to be turned on or off
        ...
        //var maxValue = 5.56;
         ...
         validValues = [0,2]; // Don't allow Freeze Protect to be turned on or off
      }
  ...
  handleMQTTMessage : function(topic, message, callback) {
    ...
    //this.log.debug("this MQTT received for '%s'. Topic:'%s' Message:'%s'", this.id, topic, message);
